...
## 結論
目前狀態：**可進入主機轉移準備階段**，但仍有 3 個外部憑證/設定項目需補齊後，才能執行全自動遷移與切流。
...
## 尚未完成（轉移前必要）
1. 缺 FTP 憑證環境變數
- 影響：無法自動上傳 sitemap 與遠端 logs 清理
- 需要：`USADA_FTP_USER`, `USADA_FTP_PASS`

2. 缺 GSC 憑證環境變數
- 影響：無法自動匯出 Search Console 關鍵字
- 需要：`GSC_SERVICE_ACCOUNT_JSON` 或有效的 `GSC_TOKEN_PATH`
...
## 轉移前最小待辦（Blocking）
- [ ] 你回傳 Linode/Cloudflare 遷移授權資料
- [ ] 設定 FTP/GSC 環境變數（或交付可讀取私密檔）
- [ ] 產出最新 DB dump + uploads 清理後備份

完成以上三點後，我可直接執行完整遷移流程。
